...
-- VERSÃO: COM LIMPEZA PRÉVIA
-- =====================================================

-- ====================================================
-- PARTE 1: LIMPEZA (Remove tudo antes de recriar)
-- ====================================================

-- Remover functions
DROP FUNCTION IF EXISTS contar_aditivos_ativos(UUID);
DROP FUNCTION IF EXISTS cancelar_aditivo_contratual(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS buscar_aditivos_contrato(UUID);
DROP FUNCTION IF EXISTS criar_aditivo_contratual(UUID, UUID, TEXT, NUMERIC, TEXT, TEXT, TEXT);

-- Remover campos adicionados em admin_parcelas_receber
ALTER TABLE admin_parcelas_receber DROP COLUMN IF EXISTS ultimo_aditivo_id;
ALTER TABLE admin_parcelas_receber DROP COLUMN IF EXISTS valor_original;

-- Remover tabela
DROP TABLE IF EXISTS admin_aditivos_contratuais CASCADE;

-- ====================================================
-- PARTE 2: CRIAÇÃO (Recria tudo do zero)
-- ====================================================

-- 1. Criar tabela de aditivos contratuais
CREATE TABLE admin_aditivos_contratuais (
...
BEGIN
  ...
  -- Buscar valor atual do contrato (soma das parcelas pendentes)
  SELECT 
    COUNT(*),
  ...
  IF v_parcelas_abertas = 0 THEN
    RAISE EXCEPTION 'Nenhuma parcela pendente encontrada para este contrato';
  END IF;
...
COMMENT ON FUNCTION criar_aditivo_contratual IS 'Cria aditivo contratual e recalcula parcelas pendentes';
