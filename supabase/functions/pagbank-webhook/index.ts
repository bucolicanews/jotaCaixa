...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (config.conta_id && config.conta_sintetica_id && parcela.admin_contas_receber.id_conta_patrimonial) {
        ...
        lancamentosPayload.push({
            ...
            valor: valorLiquido,
            tipo: 'Entrada',
            conta_bancaria_id: config.conta_id,
        ...
        throw new Error('Mapeamento contábil incompleto nas configurações do PagBank.');
      }
