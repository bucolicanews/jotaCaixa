  ...
  try {
      ...
      .select(`
        *,
        admin_contas_receber (
          *,
          tbl_clientes ( nome, email, cpf, cnpj, documento )
        )
      `)
      .eq('id', parcela_id)
      .single();
    ...
    const chargeRequest: CreateChargeRequest = {
      ...
      customer: {
        ...
        tax_id: (cliente.cpf || cliente.cnpj || cliente.documento || '').replace(/\D/g, ''),
      },
      items: [{
    ...
    return new Response(JSON.stringify({ success: true, charge_id: chargeResponse.id, qr_code: qrCode, qr_code_text: qrCodeText }), {
