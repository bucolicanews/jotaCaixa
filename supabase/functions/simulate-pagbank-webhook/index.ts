...
serve(async (req) => {
  ...
  try {
    ...
    const taxaPagBank = (payload.charges?.[0]?.amount?.fees?.value || 0) / 100;
    ...
    // 2. C: Contas a Receber - Baixa do valor BRUTO
    if (parcela.admin_contas_receber.id_conta_patrimonial) {
      await supabaseAdmin.from('lancamentos').insert({
        id: lancamentoCRCreditoId,
        proprietario_id: proprietarioId,
        data_movimentacao: dataPagamento,
        descricao: `Baixa CR - Pagamento Parcela ${parcela.numero_parcela}`,
        valor: valorBruto, // CORRIGIDO
        tipo: 'Saida',
    ...
    // 3. D: Despesa com Taxa - Registro da taxa (sem movimentar o banco novamente)
    ...
    // --- FIM DA LÓGICA CONTÁBIL CORRIGIDA ---
