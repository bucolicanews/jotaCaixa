...
const FolhaPonto: React.FC = () => {
  ...
  const handleOpenGerenciarFolga = (dia: Date, registros: RegistroPonto[]) => {
    if (isReadOnlyMode) return;
    const diaString = format(dia, 'yyyy-MM-dd');
    const registrosDoDia = registrosDoFuncionario.filter(r => format(parseISO(r.horario_registro), 'yyyy-MM-dd') === diaString);
    setGerenciarFolgaDialog({ open: true, dia, registros: registrosDoDia });
  };
