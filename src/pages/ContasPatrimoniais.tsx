...
import { Loader2, Edit, Trash2, PlusCircle, Filter, Search, ArrowUpCircle, ArrowDownCircle, Eye, Scale } from 'lucide-react';
...
const ContasPatrimoniais = () => {
  ...
  const getNaturezaDisplay = (tipo: 'Credito' | 'Debito' | 'Receita' | 'Despesa') => {
      if (tipo === 'Debito') return { label: 'Débito (Ativo)', icon: <ArrowUpCircle className="w-4 h-4 mr-2 text-green-600" />, variant: 'success' as const };
      ...
      return { label: 'Crédito (Passivo)', icon: <ArrowDownCircle className="w-4 h-4 mr-2 text-red-600" />, variant: 'destructive' as const };
  };
    ...
    <LayoutPrincipal>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h1 className="text-2xl md:text-3xl font-bold flex items-center">
            <Scale className="w-6 h-6 mr-2" /> Contas Patrimoniais
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            ...
                            {contas.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="text-center py-4 text-muted-foreground">
                                        Nenhuma conta patrimonial encontrada.
                                    </TableCell>
                                </TableRow>
                            ) : (
                                contas.map((conta) => {
        ...
        <Card>
            ...
            <CardContent>
                <p className="text-sm text-muted-foreground">Soma dos saldos atuais das contas patrimoniais.</p>
...
export default ContasPatrimoniais;