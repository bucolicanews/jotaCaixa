...
import { Ferias } from '@/types/ponto';
...
interface GerenciarFeriasAdminProps {
  funcionarioId: string;
  adminId: string; // Usando adminId como proprietário
  readOnly?: boolean;
}

const GerenciarFeriasAdmin: React.FC<GerenciarFeriasAdminProps> = ({ funcionarioId, adminId, readOnly = false }) => {
  ...
  const buscarFerias = useCallback(async () => {
    setCarregando(true);
    // Usando a tabela admin_ferias_user
    ...
    if (error) {
      showError('Erro ao carregar férias do Admin: ' + error.message);
      setFerias([]);
    } else {
  ...
  const handleAdicionarFerias = async () => {
    ...
    try {
      // Usando a tabela admin_ferias_user e admin_id
      const feriasData = {
        funcionario_id: funcionarioId,
        admin_id: adminId, // Usando adminId
      ...
      const { error } = await supabase.from('admin_ferias_user').insert(feriasData);
  ...
  const handleRemoverFerias = async (id: string) => {
    ...
    try {
      // Usando a tabela admin_ferias_user
      const { error } = await supabase.from('admin_ferias_user').delete().eq('id', id);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Gestão de Férias (Admin)</CardTitle>
      </CardHeader>
...
export default GerenciarFeriasAdmin;