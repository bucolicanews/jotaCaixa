...
interface GerenciarFeriasAdminProps {
  ...
  adminId: string; // ID do Admin proprietário
  readOnly?: boolean;
}
...
const GerenciarFeriasAdmin: React.FC<GerenciarFeriasAdminProps> = ({ funcionarioId, adminId, readOnly = false }) => {
  ...
  const tabelaFerias = 'admin_ferias_user';
  ...
  const handleAdicionarFerias = async () => {
    ...
    try {
      const feriasData = {
        ...
        admin_id: adminId, // Usando admin_id
        data_inicio: format(novaDataInicio, 'yyyy-MM-dd'),
        data_fim: format(novaDataFim, 'yyyy-MM-dd'),
      };

      const { error } = await supabase.from(tabelaFerias).insert(feriasData);
  ...
  const handleRemoverFerias = async (id: string) => {
    ...
    setIsSubmitting(true);
    try {
      const { error } = await supabase.from(tabelaFerias).delete().eq('id', id);
