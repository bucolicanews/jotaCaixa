...
const FormContasReceber: React.FC<FormContasReceberProps> = ({ contaInicial, onSaveComplete }) => {
  ...
  const getOwnerId = () => {
    if (isDirectAdmin) return usuario?.id || null;
    if (isAdminUsuario) return adminIdFromProfile;
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      let tabelaContasReceber = isAdminOrEmployee ? 'admin_contas_receber' : 'contas_receber';
      let tabelaParcelasReceber = isAdminOrEmployee ? 'admin_parcelas_receber' : 'parcelas_contas_receber';
      ...
      const parcelasComId = parcelasParaInserir.map(p => ({ 
          ...
          ...(isAdminOrEmployee ? { admin_id: ownerId } : { empresa_id: ownerId }),
