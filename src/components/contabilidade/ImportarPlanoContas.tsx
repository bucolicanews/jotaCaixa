...
<dyad-write path="src/components/contabilidade/MapearTodasFKsDialog.tsx" description="Atualizando o diálogo de mapeamento de FKs para incluir a nova lógica de atualização de lançamentos.">
...
const MapearTodasFKsDialog: React.FC<MapearTodasFKsDialogProps> = ({
    ...
    const handleConfirmImport = async () => {
        ...
        try {
            ...
            // NOTA: Esta etapa foi movida para o frontend antes de abrir o modal no ImportarPlanoContas.tsx
            // Mas a Edge Function de update-plano-contas-fks fará o upsert final.
...
<dyad-write path="src/components/contabilidade/EditableCell.tsx" description="Atualizando o componente EditableCell para usar a nova tipagem de PlanoContas.">
...
interface EditableCellProps {
  ...
  fieldName: keyof PlanoContas; // Nome do campo a ser atualizado (Conta, Descricao, codigo_reduzido, is_conta_caixa_banco, is_conta_patrimonial, is_conta_resultado, is_caixa, is_banco, is_a_receber, is_a_pagar)
  onSaveSuccess: () => void;
  className?: string;
...
const EditableCell: React.FC<EditableCellProps> = ({
  ...
  const handleSave = async (newValue?: boolean) => {
    ...
    const payload: Partial<PlanoContas> = {
        [fieldName]: finalValue,
        atualizado_em: new Date().toISOString(),
    };
    
    // Lógica de sincronização de flags (is_caixa/is_banco -> is_conta_caixa_banco)
    if (fieldName === 'is_caixa' || fieldName === 'is_banco') {
        const currentConta = (await supabase.from('plano_contas').select('is_caixa, is_banco').eq('id', id).single()).data;
        const isCaixa = fieldName === 'is_caixa' ? (finalValue as boolean) : currentConta?.is_caixa;
        const isBanco = fieldName === 'is_banco' ? (finalValue as boolean) : currentConta?.is_banco;
        
        payload.is_conta_caixa_banco = isCaixa || isBanco;
    }
