...
sql_lines = []
sql_lines.append("-- SQL para inserir lancamentos de Pagamentos com partida dobrada CORRIGIDA")
sql_lines.append(f"-- Total: {len(pagamentos)} registros × 2 lancamentos\n")
...
for i, pag in enumerate(pagamentos, 1):
    ...
    # Lançamento 1: SAIDA em Conta de Resultado (Despesa fica credora)
    ...
    # Lançamento 2: SAIDA em Conta Bancaria (banco é devedora, saida diminui)
    l2 = f"INSERT INTO lancamentos (id, proprietario_id, data_movimentacao, valor, conta_contabil_id, tipo, historico_id, origem, conciliado, conta_bancaria_id, documento, anexo_id, conta_resultado_id) VALUES (gen_random_uuid(), '{propriet_id}', '{data}', {valor}, '{conta_banco}', 'Saida', {historico_val}, '{origem}', false, null, null, null, null);"
    sql_lines.append(l2)
    count += 1

with open('pagamentos_lancamentos_corrigido.sql', 'w', encoding='utf-8') as f:
    f.write('\n'.join(sql_lines))
    f.write(f"\n\n-- Total de lancamentos: {count}\n")

print(f"[OK] SQL gerado: pagamentos_lancamentos_corrigido.sql ({count} INSERTs)")
